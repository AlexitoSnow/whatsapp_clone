...
import '../../../common/repositories/common_storage_repository.dart';
  ...
  Stream<List<Message>> getChatStream(String recieverUserId) {
    return firestore
      ...
      return List.generate(
        event.docs.length,
        (index) => Message.fromMap(event.docs[index].data()),
      );
    });
  }
...
// users -> reciever user id => chats -> current user id -> set data
      ...
      await firestore
          ...
          .then((_) {
        log('Data saved to $recieverUserId chats subcollection',
            name: 'ChatRepository');
      });
      ...
      await firestore
          ...
          .then((_) {
        log('Data saved to ${auth.currentUser!.uid} chats subcollection',
            name: 'ChatRepository');
      });
    }
  }
  ...
  void _saveMessageToMessageSubcollection({
    ...
    if (isGroupChat) {
      ...
      await firestore
          ...
          .then((_) {
        log('Data saved to $recieverUserId messages subcollection',
            name: 'ChatRepository');
      });
    }
  }
    ...
    try {
      ...
      var imageUrl =
          await ref.read(commonStorageRepositoryProvider).storeFileToFirebase(
                'chat/${messageType.name}/${senderUserData.uid}/$recieverUserId/$messageId',
                file,
              );
  ...
  Future<String?> setChatMessageSeen(
    String recieverUserId,
    String messageId,
  ) async {
    try {
